...
const get = (id: number): Promise<Recipe> => http.get<Recipe>(`/Recipes/id?id=${id}`);

const getList = (pageNumber: number, fare: Fare): Promise<PaginatedList<RecipeListItem>> => http.get<Recipe>(`/Recipes/getList?pageNumber=${pageNumber}&pageSize=10&fare=${fare}`);
...
const del = (id: number): Promise<void> => http.delete(`/Recipes/id?id=${id}`);
